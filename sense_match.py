...
def parse(root):                                 # Function for parsing the propbank XML file and retrieving data from it
	data = {}
	if root == None:
		return
...
if __name__ == "__main__":
	...
	word_matcher = spacy.load("en_core_web_md")      # Loading SpaCy word matcher
	query = str(sys.argv[1])
	tree = ET.parse("propbank_frames/"+query+".xml")
	root = tree.getroot()
	#print(parse(root))
	data = parse(root)
	#print(data)
	...
	synset = {}
	for onto in ontologies:                            # Retrieving synsets from ontogies of the word and pre-processing them
	...
	for word in data.keys():
		if word == query:
			for roleset in data[word].keys():           # Retrieving components in roleset definition and other elements
				...
				list_of_synonyms = []
				for item in list_of_words:              # Extracting synsets from WordNet for each roleset component
					try:
				...
				max_match = 0
				result = None
				for onto in synset.keys():             # Sense matching and similarity score computation
					similarity = 0
					matches = []
					matched = False
